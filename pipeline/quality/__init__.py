...
class DQReport:
    ...
    def summary_text(self) -> str:
        ...
        for c in self.checks:
            ...
            lines.append(f"  {icon}  [{c.category:12s}] {c.check_id:45s} {c.description}{fail_info}")
...
class DataQualityEngine:
    ...
    def run_all(self, run_id: str | None = None) -> DQReport:
        ...
        logger.info("")
        logger.info("DQ COMPLETE  passed=%d  failed=%d  total=%d  elapsed=%.3fs",
                     report.passed, report.failed, report.total, elapsed)
        return report

    def _check_nulls(self, report: DQReport):
        logger.info("─ 5.1  Null checks ─")
        required_columns = {
            "gold.dim_customers": ["customer_sk", "customer_id", "canonical_customer_id", "email"],
            "gold.dim_products": ["product_sk", "product_id", "sku", "product_name"],
            "gold.dim_date": ["date_key", "full_date"],
            "gold.fact_orders": ["order_sk", "order_id", "customer_sk", "order_date_key",
                                 "order_status", "currency", "total_amount"],
            "gold.fact_order_items": ["order_item_sk", "order_item_id", "order_sk",
                                      "product_sk", "quantity", "unit_price"],
            "gold.fact_payments": ["payment_sk", "transaction_id", "order_sk",
                                   "payment_method", "status", "currency", "gross_amount"],
            "gold.fact_inventory_daily": ["inventory_sk", "snapshot_date_key",
                                          "product_sk", "warehouse_id", "on_hand_qty"],
            "gold.fact_campaign_performance": ["campaign_perf_sk", "campaign_date_key",
                                                "platform", "campaign_id", "impressions"],
            "gold.fact_email_engagement": ["email_sk", "send_date_key", "customer_sk",
                                           "delivery_status"],
            "gold.fact_purchase_orders": ["po_sk", "po_id", "product_sk", "ordered_qty"],
            ...
            "silver.stg_payment_transactions": ["transaction_id", "order_id", "currency"],
        }
    ...
    def _check_schema(self, report: DQReport):
        ...
        expected_schemas: dict[str, list[tuple[str, str]]] = {
            "gold.dim_customers": [
                ("customer_sk", "INTEGER"), ("customer_id", "INTEGER"),
                ("canonical_customer_id", "INTEGER"), ("email", "VARCHAR"),
            ],
            "gold.dim_products": [
                ("product_sk", "INTEGER"), ("product_id", "INTEGER"),
                ("sku", "VARCHAR"), ("product_name", "VARCHAR"),
            ],
            "gold.fact_orders": [
                ("order_sk", "INTEGER"), ("order_id", "INTEGER"),
                ("customer_sk", "INTEGER"), ("order_date_key", "INTEGER"),
                ("currency", "VARCHAR"), ("total_amount", "DECIMAL(18,2)"),
                ("total_amount_usd", "DECIMAL(18,2)"),
            ],
            "gold.fact_order_items": [
                ("order_item_sk", "INTEGER"), ("order_sk", "INTEGER"),
                ("product_sk", "INTEGER"), ("quantity", "INTEGER"),
                ("unit_price", "DECIMAL(18,2)"), ("line_total", "DECIMAL(18,2)"),
            ],
            "gold.fact_payments": [
                ("payment_sk", "INTEGER"), ("transaction_id", "VARCHAR"),
                ("order_sk", "INTEGER"), ("currency", "VARCHAR"),
                ("gross_amount", "DECIMAL(18,2)"),
            ],
            "gold.fact_inventory_daily": [
                ("inventory_sk", "INTEGER"), ("snapshot_date_key", "INTEGER"),
                ("product_sk", "INTEGER"), ("on_hand_qty", "INTEGER"),
            ],
            "gold.fact_campaign_performance": [
                ("campaign_perf_sk", "INTEGER"), ("campaign_date_key", "INTEGER"),
                ("platform", "VARCHAR"), ("impressions", "INTEGER"),
            ],
            "gold.fact_email_engagement": [
                ("email_sk", "INTEGER"), ("send_date_key", "INTEGER"),
                ("customer_sk", "INTEGER"), ("delivery_status", "VARCHAR"),
            ],
            "gold.fact_purchase_orders": [
                ("po_sk", "INTEGER"), ("po_id", "INTEGER"),
                ("product_sk", "INTEGER"), ("ordered_qty", "INTEGER"),
            ],
        }
    ...
    def _check_duplicates(self, report: DQReport):
        ...
        for table, col in unique_keys:
            ...
            distinct = self._query_scalar(f'SELECT COUNT(DISTINCT "{col}") FROM {table}')
            dups = total - distinct
            report.checks.append(CheckResult(
    ...
    def _check_referential_integrity(self, report: DQReport):
        ...
        fk_checks = [
            ...
            ("gold.fact_inventory_daily", "product_sk", "gold.dim_products", "product_sk"),
            ("gold.fact_inventory_daily", "snapshot_date_key", "gold.dim_date", "date_key"),
            ("gold.fact_campaign_performance", "campaign_date_key", "gold.dim_date", "date_key"),
            ("gold.fact_email_engagement", "customer_sk", "gold.dim_customers", "customer_sk"),
            ("gold.fact_email_engagement", "send_date_key", "gold.dim_date", "date_key"),
            ("gold.fact_purchase_orders", "product_sk", "gold.dim_products", "product_sk"),
            ("gold.fact_purchase_orders", "created_date_key", "gold.dim_date", "date_key"),
        ]
...
def write_report(report: DQReport) -> Path:
    ...
    with open(csv_path, "a", newline="") as f:
        ...
        writer.writerow([report.run_id, report.run_ts, report.total, report.passed, report.failed])
